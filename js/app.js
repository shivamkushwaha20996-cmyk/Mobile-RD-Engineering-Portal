...
function renderCard(key,item){const color=CATEGORY_COLORS[item.category]||CATEGORY_COLORS.Specification,tags=(item.tags||[]).map(t=>`<span class="tag">${escapeHtml(t)}</span>`).join("");const favKey=`${currentModel}:${key}`,isFav=favorites.includes(favKey);const uploadedName=item.filename||"";const uploadedSize=item.size||"";let files;if(key===HWC_KEY){files=uploadedName?`<div class="file-row"><div class="min-w-0"><div class="file-name">${escapeHtml(uploadedName)}</div><div class="file-size">${escapeHtml(uploadedSize||"FILE")}</div></div><button class="download-btn" data-download="${currentModel}_${key}" data-filename="${escapeHtml(uploadedName)}"><i class="fa-solid fa-download"></i> DOWNLOAD</button></div>`:`<div class="file-row"><div class="min-w-0"><div class="file-name">No checklist uploaded</div><div class="file-size">Upload the active model hardware checklist</div></div><button class="download-btn hw-upload-btn" data-hw-upload="1"><i class="fa-solid fa-cloud-arrow-up"></i> UPLOAD</button></div>`}else if(uploadedName){files=`<div class="file-row"><div class="min-w-0"><div class="file-name" title="${escapeHtml(uploadedName)}">${escapeHtml(uploadedName)}</div><div class="file-size">${escapeHtml(uploadedSize||"Uploaded file")}</div></div><button class="download-btn" style="background:${color}" data-download="${currentModel}_${key}" data-filename="${escapeHtml(uploadedName)}"><i class="fa-solid fa-download"></i> DOWNLOAD</button></div>`}else if(item.subItems?.length){files=item.subItems.map(s=>`<div class="file-row"><div class="min-w-0"><div class="file-name">${escapeHtml(s.name)}</div><div class="file-size">${escapeHtml(s.size||"Reference")}</div></div><span class="file-row-state">REFERENCE</span></div>`).join("")}else{files=`<div class="file-row"><div class="min-w-0"><div class="file-name">No file uploaded</div><div class="file-size">Use Admin → Upload to attach a file</div></div><button class="download-btn" style="background:${color}" data-open-upload="${escapeHtml(key)}"><i class="fa-solid fa-cloud-arrow-up"></i> UPLOAD</button></div>`}return `<article id="record-${escapeHtml(key)}" class="record-card ${key===HWC_KEY?"hardware-checklist-record ":""}${isFav?"is-favorite":""}" data-record="${escapeHtml(key)}"><div class="record-stripe" style="background:${color}"></div><div class="record-body"><div><div class="record-header"><div class="record-title-wrap"><div class="record-icon" style="background:${color}"><i class="fa-solid ${escapeHtml(item.icon)}"></i></div><div class="min-w-0"><div class="record-key">${"RECORD "+escapeHtml(key)}</div><div class="record-title">${escapeHtml(item.title)}</div></div></div><div class="card-actions"><span class="category-label">${escapeHtml(item.category)}</span><button class="favorite-btn ${isFav?"active":""}" data-favorite="${escapeHtml(favKey)}" title="Favorite"><i class="fa-${isFav?"solid":"regular"} fa-star"></i></button></div></div><div class="tags">${tags||'<span class="tag">No additional metadata</span>'}</div></div><div class="file-area">${files}</div><div class="card-footer-actions"><button class="text-action" data-open-record="${escapeHtml(key)}"><i class="fa-solid fa-up-right-and-down-left-from-center"></i> Open</button><button class="text-action" data-copy-record="${escapeHtml(key)}"><i class="fa-regular fa-copy"></i> Copy</button><button class="text-action" data-link-record="${escapeHtml(key)}"><i class="fa-solid fa-link"></i> Link</button></div></div></article>`}
...
function bindEvents(){
  ...
  document.getElementById("cardsGrid").addEventListener("click",e=>{const hw=e.target.closest("[data-hw-upload]");if(hw){document.getElementById("uploadRecord").value=HWC_KEY;openModal("uploadModal");return}const fav=e.target.closest("[data-favorite]");if(fav){toggleFavorite(fav.dataset.favorite);return}const d=e.target.closest("[data-download]");if(d){downloadRecord(d.dataset.download,d.dataset.filename);return}const up=e.target.closest("[data-open-upload]");if(up){document.getElementById("uploadModel").value=currentModel;document.getElementById("uploadRecord").value=up.dataset.openUpload;openModal("uploadModal");return}const o=e.target.closest("[data-open-record]");if(o){openRecord(o.dataset.openRecord);openPresentation(o.dataset.openRecord);return}const c=e.target.closest("[data-copy-record]");if(c){copyRecord(c.dataset.copyRecord);return}const l=e.target.closest("[data-link-record]");if(l){copyLink(l.dataset.linkRecord);return}const clr=e.target.closest("[data-clear-filters]");if(clr)clearFilters()});
