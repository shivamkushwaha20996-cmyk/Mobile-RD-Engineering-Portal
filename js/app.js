...
let data=loadData(), prefs=loadPrefs(), currentModel=MODEL_ORDER[0], activeCategory="all", query="", sortMode="default", favoritesOnly=false, recentlyViewed=JSON.parse(localStorage.getItem("MOBILE_RND_RECENT_V1")||"[]"), isAdmin=sessionStorage.getItem(AUTH_KEY)==="true", selectedFile=null, portalLocked=sessionStorage.getItem("MOBILE_RND_PORTAL_LOCKED_V1")==="1";
...
function bindEvents(){
  ...
  document.getElementById("copySpecsBtn").addEventListener("click",copySpecs);document.getElementById("exportJsonBtn").addEventListener("click",exportModel);document.getElementById("exportReportBtn").addEventListener("click",exportReport);document.getElementById("settingsBtn").addEventListener("click",()=>{if(!portalLocked)openModal("settingsModal")});document.getElementById("themeBtn").addEventListener("click",()=>{if(portalLocked)return;prefs.theme=prefs.theme==="dark"?"light":"dark";savePrefs();applyPrefs()});document.getElementById("lockPortalBtn").addEventListener("click",lockPortal);document.getElementById("portalUnlockForm").addEventListener("submit",unlockPortal);document.getElementById("fullscreenBtn").addEventListener("click",toggleFullscreen);
...
function applyPrefs(){
  ...
  const accentMap={cyan:["#0891b2","#ecfeff","#a5f3fc"],blue:["#2563eb","#eff6ff","#bfdbfe"],violet:["#7c3aed","#f5f3ff","#ddd6fe"],emerald:["#059669","#ecfdf5","#a7f3d0"],amber:["#d97706","#fffbeb","#fde68a"],rose:["#e11d48","#fff1f2","#fecdd3"],orange:["#ea580c","#fff7ed","#fed7aa"],indigo:["#4f46e5","#eef2ff","#c7d2fe"],teal:["#0d9488","#f0fdfa","#99f6e4"],fuchsia:["#c026d3","#fdf4ff","#f5d0fe"],sky:["#0284c7","#f0f9ff","#bae6fd"]};
  const a=accentMap[prefs.accent]||accentMap.cyan;
  for(const el of [document.documentElement,document.body]){el.style.setProperty("--accent",a[0]);el.style.setProperty("--accent-soft",a[1]);el.style.setProperty("--accent-border",a[2])}
  document.getElementById("themeSelect").value=prefs.theme;document.getElementById("accentSelect").value=prefs.accent;document.getElementById("densitySelect").value=prefs.density;document.getElementById("motionToggle").checked=prefs.motion;
  document.getElementById("themeIcon").className=`fa-solid fa-${prefs.theme==="dark"?"sun":"moon"}`;
  renderLockState();
}
function renderLockState(){const o=document.getElementById("portalLockOverlay"),b=document.getElementById("lockPortalBtn");if(o)o.classList.toggle("hidden",!portalLocked);if(b){b.title=portalLocked?"Portal is locked":"Lock portal";b.innerHTML=`<i class="fa-solid fa-${portalLocked?"lock":"lock"}"></i>`}document.body.classList.toggle("portal-is-locked",portalLocked)}
function lockPortal(){portalLocked=true;sessionStorage.setItem("MOBILE_RND_PORTAL_LOCKED_V1","1");document.querySelectorAll(".modal:not(.hidden)").forEach(m=>m.classList.add("hidden"));renderLockState();setTimeout(()=>document.getElementById("portalUnlockPassword")?.focus(),50);}
function unlockPortal(e){e.preventDefault();const input=document.getElementById("portalUnlockPassword"),err=document.getElementById("portalUnlockError");if(input.value==="admin123"){portalLocked=false;sessionStorage.removeItem("MOBILE_RND_PORTAL_LOCKED_V1");input.value="";err.classList.add("hidden");renderLockState();toast("Portal unlocked.","success")}else{err.classList.remove("hidden");input.select()}}
